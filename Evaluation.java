public class Evaluation {
    ...
    public static float evaluate(Chess board) {
        ...
        for (int i = 0; i < 64; i++)
            res += switch (board.pieceAt((byte) i)) {
                case 'P' -> PAWN + (PT[i / 8][i % 8] / 100f);
                case 'R' -> ROOK + (RT[i / 8][i % 8] / 100f);
                case 'N' -> KNIGHT + (NT[i / 8][i % 8] / 100f);
                case 'B' -> BISHOP + (BT[i / 8][i % 8] / 100f);
                case 'Q' -> QUEEN + (QT[i / 8][i % 8] / 100f);
                case 'K' -> KING + ((endGame ? KET[i / 8][i % 8] : KMT[i / 8][i % 8]) / 100f);
                case 'p' -> -PAWN - (PT[7 - (i / 8)][7 - (i % 8)] / 100f);
                case 'r' -> -ROOK - (RT[7 - (i / 8)][7 - (i % 8)] / 100f);
                case 'n' -> -KNIGHT - (NT[7 - (i / 8)][7 - (i % 8)] / 100f);
                case 'b' -> -BISHOP - (BT[7 - (i / 8)][7 - (i % 8)] / 100f);
                case 'q' -> -QUEEN - (QT[7 - (i / 8)][7 - (i % 8)] / 100f);
                case 'k' -> -KING - ((endGame ? KET[7 - (i / 8)][7 - (i % 8)] : KMT[7 - (i / 8)][7 - (i % 8)]) / 100f);
                default -> 0;
            };
        if (board.gameOver)
